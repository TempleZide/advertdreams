...
Research for [#6](https://github.com/TempleZide/advertdreams/issues/6). Prices verified **2026-08-23** against live provider pages. Telecom pricing moves — re-verify before quoting a Client. Every figure is cited to a primary source or explicitly flagged as unconfirmed.
...
### A2P 10DLC registration — required, and probably per-Client
...
### STIR/SHAKEN — the open legal question
...
**The Client consents too.** The contractor is a party to the recorded call. Capture their consent in the service agreement at onboarding rather than relying on implied consent — do not rely on the caller-side announcement to cover the Client leg.

Cost: $0 in fees, but it is a real product requirement — announcement audio played before the `<Dial>` bridges, and a consent clause in the Client agreement.

### TCPA / consent for outbound

Texting the Client Owner about their own leads is transactional, not telemarketing: they provided the number in connection with the service they are paying for, so consent is implied for messages closely related to that purpose. **Still capture explicit SMS opt-in in the signup flow** — cheap belt-and-braces for an automated message stream.
...
- **State USF / 911 / regulatory surcharges:** passed through by the provider on the number. Small, but not zero — *unconfirmed*, check the provider's fee schedule.
...
5. **Choosing Twilio means advertdreams builds the call-tracking layer itself** — forwarding TwiML, announcement, webhook handlers, number provisioning. This is small (the `Lead` model and dashboard are being built regardless per [#10](https://github.com/TempleZide/advertdreams/issues/10)) and buys full control of attribution, which is advertdreams' own model, not a vendor's.
