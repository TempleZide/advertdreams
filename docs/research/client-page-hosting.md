...
**Unconfirmed:** Cloudflare Registrar sells domains at cost, and its own page states that it "does not mark up domain prices at all… customers only pay the price charged by registries and ICANN" ([Cloudflare Registrar](https://www.cloudflare.com/products/registrar/), verified 2026-08-24), but **it does not publish the `.com` figure anywhere in its public docs or on that page** — the price appears only inside the dashboard. The Verisign `.com` wholesale registry price could not be retrieved either; Verisign's own pricing page returned no figures and ICANN's site refused automated fetches. Porkbun's $11.08 is used above because it is the cheapest number actually verified at a primary source. Cloudflare at cost would likely be a little lower, which only strengthens the conclusion. **Verisign's `.com` wholesale price rises annually under its registry agreement, so this line is the one in this document most likely to be stale first.**
